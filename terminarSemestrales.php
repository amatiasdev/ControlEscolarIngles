...
$existeGrupo="SELECT count(*) FROM grupos WHERE activo=1 and (horario='MATUTINO' or horario='VESPERTINO' or horario='INTERMEDIO')";
$respExisteGrupo=$conexion->query($existeGrupo);
$respExisteGrupo=$respExisteGrupo->fetch_assoc();
if($respExisteGrupo['count(*)']>0){

$query2="SELECT idgrupo FROM grupos WHERE activo=1 and (horario='MATUTINO' or horario='VESPERTINO' or horario='INTERMEDIO')";
...
$numeroGrupos="SELECT count(*) FROM grupos WHERE activo=1 and (horario='INTERMEDIO' or horario='MATUTINO'or horario='VESPERTINO')";
...
while ($i<=$filaNumeroGrupos['count(*)']) {
	$query2="SELECT idgrupo FROM grupos WHERE activo=1 and(horario='INTERMEDIO' or horario='MATUTINO' or horario='VESPERTINO')";
$resp=$conexion->query($query2);
